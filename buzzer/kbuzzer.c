...
int bell_ioctl(struct file *filp, unsigned int request, unsigned long arg)
{
  switch(request){
  case 0:
...
struct file_operations bell_fop = {
  .owner = THIS_MODULE,
  .open = bell_open,
  .release = bell_close,
...
static __init int bell_init(void)
{
  int ret;
  dev_t devno;
  ...
  bell_devp = kmalloc(sizeof(struct bell_dev),GFP_KERNEL);
  if(!bell_devp){
    ret = -ENOMEM;
...
static __exit void bell_exit(void)
{
...
module_init(bell_init);
module_exit(bell_exit);
