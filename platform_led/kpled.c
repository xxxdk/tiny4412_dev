...
static int led_probe(struct platform_device *pdev)
{
  ...
  led_devp = devm_kzalloc(&pdev->dev, sizeof(struct led_dev),GFP_KERNEL);
  if(!led_devp){
    ret = -ENOMEM;
  ...
  // led_dat = (volatile unsigned long *)ioremap(GPM4COM+1,4);
